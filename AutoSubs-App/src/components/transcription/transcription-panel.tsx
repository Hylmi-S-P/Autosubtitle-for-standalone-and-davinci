...
import { Speech, Type, AudioLines, Globe, X, PlayCircle, Container } from "lucide-react"
      ...
      <div className="flex-shrink-0">
        <Card className="p-3 sticky bottom-4 mx-4 z-50 shadow-lg bg-card">
          <div className="grid w-full gap-3">
            ...
            {/* Local AI (Docker) toggle */}
            <div className="flex items-center justify-between rounded-lg border border-border/50 bg-muted/30 px-3 py-2 mt-1">
              <div className="flex items-center gap-2">
                <Container className="h-4 w-4 text-muted-foreground" />
                <Label
                  htmlFor="use-local-ai"
                  className="text-xs font-medium cursor-pointer select-none"
                >
                  Use Local AI
                  <span className="text-muted-foreground font-normal ml-1">
                    (Faster-Whisper via Docker)
                  </span>
                </Label>
              </div>
              <Switch
                id="use-local-ai"
                checked={useLocalAI ?? false}
            ...
            {isProcessing ? (
              ...
              <Button
                onClick={onStart}
                size="default"
                variant={useLocalAI ? "secondary" : "default"}
                className={`w-full mt-1 ${useLocalAI ? "border border-primary/50 bg-primary/10 hover:bg-primary/20 text-primary" : ""}`}
                disabled={isProcessing}
              >
                {useLocalAI ? (
                ...
                {useLocalAI ? "Generate with Docker AI" : t("common.generateSubtitles")}
              </Button>
            )}
          </div>
        </Card>
...
export function TranscriptionPanel({ onViewSubtitles }: { onViewSubtitles?: () => void } = {}) {
  ...
  const [useLocalAI, setUseLocalAI] = React.useState(false)
    ...
    try {
      if (useLocalAI) {
        ...
        try {
          ...
          // Parse the raw SRT response using the app's existing parser
          const { parseSrt } = await import("@/utils/srt-utils")
          const parsedSegments = parseSrt(srtText)

          if (parsedSegments.length === 0) {
            throw new Error("Docker API returned empty or unparseable SRT data.")
          }

          // Wrap in the same transcript shape that processTranscriptionResults expects
          const transcript = { segments: parsedSegments }
          ...
          toast.success("Docker transcription completed!", {
            description: `Generated ${parsedSegments.length} subtitle segments.`,
            duration: 5000,
          })
        } catch (dockerError: any) {
          console.error("Docker transcription failed:", dockerError)
          cancelAllProgressSteps()
          toast.error("Docker AI Transcription Failed", {
            description:
              typeof dockerError === "string"
                ? dockerError
                : dockerError?.message ?? "Is your Docker container running on localhost:8000?",
            duration: 8000,
          })
        }
      } else {
        // === Built-in transcription engine route (unchanged) ===
